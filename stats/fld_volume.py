...
def render_fld_volume(fld: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        with tab_bar:
            st.altair_chart(_build_fld_volume_bar(df, title="Cases Filed", subtitle=subtitle))
        with tab_line:
            st.altair_chart(_build_fld_volume_line(df))
...
def render_file_rate(fld: pd.DataFrame, ntfld: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        st.altair_chart(_build_file_rate_line(df))
...
def render_decision_time(fld: pd.DataFrame, ntfld: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        st.altair_chart(_build_decision_time_line(df))
