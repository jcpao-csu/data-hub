...
def render_ntfld_refiled(ntfld: pd.DataFrame, fld_all: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        with tab_count:
            st.altair_chart(_build_refiled_bar(melted, is_normalized=False), use_container_width=True)
        with tab_pct:
            st.altair_chart(_build_refiled_bar(melted, is_normalized=True), use_container_width=True)
        with tab_rate:
            st.altair_chart(_build_refiled_line(wide), use_container_width=True)
        ...
        st.altair_chart(_build_refiled_by_reason(reason_df), use_container_width=True)
