...
def render_ntfld_volume(ntfld: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        with tab_bar:
            st.altair_chart(_build_ntfld_volume_bar(df, title="Cases Not Filed", subtitle=subtitle), use_container_width=True)
        with tab_line:
            st.altair_chart(_build_ntfld_volume_line(df), use_container_width=True)
