...
from session_state import initial_session_state, pages_session_state, initiate_widgets, sidebar_widgets
...
with st.sidebar:
    ...
    st.write(f"Welcome to the JCPAO Dashboard! Results based on system data as of {post_last_updated(RCVD)}.") # Please use the interactive widgets below to explore the dashboard.")
    st.divider()
...
# session_state callback func (update_df) handles filtering and updating of dataframes to display 
def stat_func(
    df: pd.DataFrame,
    date_col: str, # datetime col to run dt.to_period() for dashboard viz
    date_range: tuple = st.session_state["date_range_filter"], # date range to view
    period_view: str = st.session_state["period_freq_filter"], # period length 
    charge_cat: str = st.session_state["charge_category_filter"], # charge category
    agency: str = st.session_state["police_agency_filter"], # agency
):
    # Prepare DF datetime col 
    df[date_col] = pd.to_datetime(df[date_col])
    df["groupby_period"] = df[date_col].dt.to_period(period_view) # .dt.to_timestamp()

    # Groupby DF 
    groupby_df = df.groupby("groupby_period")["pbk_num"].nunique()
    # groupby_df["groupby_period"] = groupby_df["groupby_period"].dt.to_timestamp()
    # groupby_df["groupby_period"] = pd.to_datetime(groupby_df["groupby_period"])

    # Render Altair Chart
    chart = (
        alt.Chart(groupby_df)
        .mark_bar()
        .encode(
            x="groupby_period:T",
            y="pbk_num:O",
            tooltip=["groupby_period:T", "pbk_num:O"]
        )
    )

    st.write(groupby_df)
    st.write(f"From {date_range[0]} to {date_range[1]}, the JCPAO ...")
    st.altair_chart(chart, use_container_width=True)
    return chart


stat_func(st.session_state["data"][0], "ref_date")
